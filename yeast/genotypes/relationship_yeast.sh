###########################################################
###                                                     ###
###     Generate binary genotype files and estimate     ###
###     relationship of samples in F2 yeast cross       ###
###     from Bloom et al (2013)                         ###
###                                                     ###
###     * Prune genotypes for SNPs in ld via plink 1.9  ###
###       using a number of different window sizes      ###
###     * for each set                                  ###
###         * compute pca                               ###
...
for kb in 3 5 8 10 12 15 20 30 50 100; do
    ...
    plink --bfile $dir/$geno \
        --indep-pairwise ${kb}kb 5 0.2 \
        --out $dir/$geno.${kb}kb
    mv $dir/$geno.${kb}kb.log $dir/log/$geno.${kb}kb.prune.log

    plink --bfile $dir/$geno \
        --extract $dir/$geno.${kb}kb.prune.in \
        --make-bed \
        --out $dir/$geno.${kb}kb.pruned
    mv $dir/$geno.${kb}kb.pruned.log $dir/log/$geno.${kb}kb.pruned.log

    ### 3. run pca
    plink --bfile $dir/$geno \
        --extract $dir/$geno.prune.in \
        --pca --out $dir/$geno.pca
    ...
    Rscript ~/analysis/GWAS/population_structure.R --vanilla  \
        --default-packages=R.utils --data=$dir/$geno
